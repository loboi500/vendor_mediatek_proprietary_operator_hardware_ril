...
#define RFX_LOG_TAG "RtmOpCapa"
/*****************************************************************************
 * Class RtmCapabilitySwitchChecker
 *****************************************************************************/
RFX_IMPLEMENT_CLASS("RtmOpCapabilitySwitchChecker", RtmOpCapabilitySwitchChecker, RtmCapabilitySwitchChecker);

RtmOpCapabilitySwitchChecker::RtmOpCapabilitySwitchChecker() {
}

RtmOpCapabilitySwitchChecker::~RtmOpCapabilitySwitchChecker() {
}

int RtmOpCapabilitySwitchChecker::getHigherPrioritySlot() {
    int current_major_slot = m_status_managers[MAX_RFX_SLOT_ID]->getIntValue(
            RFX_STATUS_KEY_MAIN_CAPABILITY_SLOT, 0);

    if (RtmCapabilitySwitchUtil::isSimSwitchEnabled() == false) {
        ...
        if (sim_state != RFX_SIM_STATE_ABSENT) {
            ...
            if (RtmCapabilitySwitchUtil::isOp01Sim(getImsi(i))) {
                ...
                op01_sim_slot_id = i;
                      ...
                      RtmCapabilitySwitchUtil::isOp09Sim(getImsi(i))) {
                c2k_sim_count++;
                c2k_sim_slot_id = i;
    ...
    if (isSupportSimSwitchEnhancement(ENHANCEMENT_T_PLUS_W) && (inserted_sim_count == 2) &&
        ...
        if (RatConfig_isTdscdmaSupported()) {
            // t + empty  --> always on t card if support TD-SCDMA
            return op01_sim_slot_id;
        } else {
            // t + empty --> don't need to capability switch if not support TD-SCDMA
            return CURRENT_SLOT;
        }
    } else if ((inserted_sim_count == 1) && (c2k_sim_count == 1)) {
        if (RatConfig_isC2kSupported()) {
            // c + empty --> always on c card if support C2K
            return c2k_sim_slot_id;
        } else {
            // t + empty --> don't need to capability switch if not support C2K
            return CURRENT_SLOT;
        }
    }
    return UNKNOWN_SLOT;
...
bool RtmOpCapabilitySwitchChecker::isSkipCapabilitySwitch(int new_major_slot) {
    int current_major_slot = m_status_managers[MAX_RFX_SLOT_ID]->getIntValue(
            RFX_STATUS_KEY_MAIN_CAPABILITY_SLOT, 0);
    int sim_count = RfxRilUtils::rfxGetSimCount();
    bool ret = false;
...
bool RtmOpCapabilitySwitchChecker::isSatisfyOperatorRules(int new_major_slot) {
    bool ret = true;
...
bool RtmOpCapabilitySwitchChecker::isSatisfyOp01Rules(int new_major_slot) {
    return true;
}
